    ...
    return {
        ...
        actions: {
            ...
            fetchIndividualCharacterDetails: (characterId) => {
                fetch(`https://swapi.dev/api/people/${characterId}/`).then(response => {
                    if (response.ok) {
                        return response.json();
                    } else {
                        throw new Error("Error fetching character details");
                    }
                }).then(characterData => { // This .then block receives the JSON data of the character details.
            ...
            fetchIndividualPlanetDetails: (planetId) => {
                fetch(`https://swapi.dev/api/planets/${planetId}/`).then(response => {
                    if (response.ok) {
                        return response.json();
                    } else {
            ...
            fetchIndividualVehicleDetails: (vehicleId) => {
                fetch(`https://swapi.dev/api/vehicles/${vehicleId}/`).then(response => {
                    if (response.ok) {
                        return response.json();
                    } else {
            ...
            addFavorite: (item) => {
                ...
                const updatedFavorites = [
                    ... store.favoriteItems,
                    item
                ]; // create a new array, spreads the existing array of favorite characters and adds adds the new characterName to the end
